# 此文件中的所有代码来自 telegram-bili-feed-helper by @simonsmh.
# 感谢贡献。
...
class dynamic(feed):
    ...
    def has_forward(self):
        return bool(
            self.detailcontent.get("data").get(
                "card").get("desc").get("orig_type")
        )
...
async def dynamic_parser(s, url):
    ...
    try:
        f.type = f.detailcontent.get("data").get(
            "card").get("desc").get("type")
    except AttributeError:
        logger.warning(f"动态解析错误: {url}")
        return
    f.dynamic_id = (
        f.detailcontent.get("data").get("card").get(
    ...
    # extra parsers
        ...
        *detail_types_list.get("LIVE"),
        ...
        # bv video
            ...
            f.content = f.card.get("new_desc") if f.card.get(
                "new_desc") else fu.content
        f.user = fu.user
        f.uid = fu.uid
    ...
    # dynamic images/videos
        ...
        *detail_types_list.get("CLIP"),
        ...
        if f.origin_type in detail_types_list.get("PIC"):
            f.mediaurls = [t.get("img_src")
                           for t in f.card.get("item").get("pictures")]
            f.mediatype = "image"
